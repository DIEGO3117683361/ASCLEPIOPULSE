...
import { Routine, Exercise, PulseExerciseGoal } from '../types';
...
const SetPulseGoals: React.FC = () => {
    const navigate = useNavigate();
    const { id } = useParams<{ id: string }>();
    const { currentUser, pulses, routines, updatePulseParticipantGoals } = useUser();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        await updatePulseParticipantGoals(pulse.id, currentUser.id, goals);
        navigate(`/pulse/${pulse.id}`);
    };
    
    if (!pulse || !routine) {
        return <div className="p-4 text-center">Pulse o rutina no encontrados.</div>;
    }
    ...
    return (
        <div className="bg-graphite text-white">
            <header className="p-4 flex items-center sticky top-0 bg-graphite z-10 border-b border-graphite-lighter">
                <button onClick={() => navigate('/vs')} className="p-2 -ml-2 mr-2"><ArrowLeft /></button>
                <h1 className="text-xl font-bold">Metas para: {pulse.name}</h1>
            </header>
            ...
            <main className="p-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button type="submit" className="w-full bg-accent-lime text-graphite font-bold py-4 rounded-lg flex items-center justify-center transition hover:bg-lime-500 mt-4">
                        <Check size={20} className="mr-2"/> Guardar Metas e Iniciar
                    </button>
                </form>
            </main>
        </div>
