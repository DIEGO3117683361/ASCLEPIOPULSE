...
const Profile: React.FC = () => {
    ...
    const { users, currentUser, routines, logout, followRoutine, unfollowRoutine, toggleActiveRoutine, deleteRoutine } = useUser();
                                        ...
                                        <div className="border-t border-graphite-lighter mt-3 pt-3 flex items-center space-x-2">
                                            {!isCurrentUserProfile && (
                                                !isFollowedByCurrentUser ? (
                                                    <button onClick={() => followRoutine(r.id)} className="flex-1 text-center text-sm font-semibold bg-accent-lime/20 text-accent-lime py-2 rounded-md hover:bg-accent-lime/40 transition">Seguir Rutina</button>
                                                ) : (
                                                     <button onClick={() => unfollowRoutine(r.id)} className="flex-1 text-center text-sm font-semibold bg-red-500/20 text-red-400 py-2 rounded-md hover:bg-red-500/40 transition">Dejar de Seguir</button>
                                                )
                                            )}
                                            {isCurrentUserProfile && (
                                                <>
                                                    <button onClick={() => toggleActiveRoutine(r.id)} className={`flex-1 text-center text-sm font-semibold py-2 rounded-md transition ${isActive ? 'bg-accent-lime/20 text-accent-lime' : 'bg-graphite-lighter text-gray-300'}`}>
                                                        {isActive ? 'Desactivar' : 'Activar'}
                                                    </button>
                                                    <Link to={`/routine/edit/${r.id}`} className="flex-1 text-center text-sm font-semibold bg-graphite-lighter text-gray-300 py-2 rounded-md hover:bg-graphite-lighter/80 transition">Editar</Link>
                                                    <button onClick={() => handleDeleteClick(r.id)} className="flex-1 text-center text-sm font-semibold bg-red-500/20 text-red-400 py-2 rounded-md hover:bg-red-500/40 transition flex items-center justify-center"><Trash2 size={14} className="mr-1.5"/>Eliminar</button>
                                                </>
                                            )}
                                        </div>
                                    </div>
