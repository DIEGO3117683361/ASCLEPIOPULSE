...
import { Flame, Dumbbell, Calendar, CheckCircle, Plus, Compass, Swords, ArrowRight, ExternalLink, MessageSquare } from 'lucide-react';
import { useUser } from '../context/UserContext';
import { Exercise, Routine, RoutineDay, Announcement, ChatRoom } from '../types';
...
const TodayRoutineCard: React.FC<TodayRoutineCardProps> = ({ routine, todayRoutineDay }) => (
    <div className="bg-graphite-light p-4 rounded-xl shadow-lg">
        ...
        <div className="space-y-3">
            ...
            <Link to="/log-activity" className="block w-full text-center bg-accent-lime text-graphite font-bold py-4 rounded-lg mt-4 transition hover:bg-lime-500 flex items-center justify-center">
               <Plus size={20} className="mr-2"/> Registrar Actividad
            </Link>
        </div>
    </div>
);

const PulseHomeCard: React.FC<{ pulseId: number, pulseName: string }> = ({ pulseId, pulseName }) => (
    <div className="bg-gradient-to-br from-accent-lime/20 to-graphite-light p-4 rounded-xl shadow-lg border border-accent-lime/30">
        ...
        <Link to={`/pulse/${pulseId}/log`} className="block w-full text-center bg-accent-lime text-graphite font-bold py-4 rounded-lg transition hover:bg-lime-500 flex items-center justify-center">
           <Plus size={20} className="mr-2"/> Registrar en Pulse
        </Link>
    </div>
);
    ...
    return (
        <div className="p-4 space-y-6">
            ...
            {pulsesForToday.length > 0 && (
                pulsesForToday.map(p => p && <PulseHomeCard key={p.id} pulseId={p.id} pulseName={p.name} />)
            )}
            ...
            {activeRoutines.length === 0 && pulsesForToday.length === 0 && pulseInvitations.length === 0 && activeAnnouncements.length === 0 && totalUnreadCount === 0 ? (
                ...
                routinesForToday.map(item => 
                    item && <TodayRoutineCard key={item.routine.id} routine={item.routine} todayRoutineDay={item.todayRoutineDay} />
                )
            ) : (
                pulsesForToday.length === 0 && (
                    <div className="bg-graphite-light p-4 rounded-xl shadow-lg text-center py-8">
