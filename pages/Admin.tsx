...
import { LogOut, Users, Megaphone, Search, X, Trash2, Edit, CheckCircle, XCircle, ToggleLeft, ToggleRight, Plus, MessageSquare, Palette, Save, ShieldCheck, Flame, Send, BellRing } from 'lucide-react';
...
const PushNotificationManagement: React.FC = () => {
    ...
    const handleSend = async () => {
        if (selectedIds.length === 0 || !title.trim() || !message.trim()) {
            alert("Completa todos los campos y selecciona al menos un destinatario.");
            return;
        }
        setIsSending(true);
        try {
            const notificationsCollection = collection(db, 'system_notifications');
            ...
            alert(`Notificación enviada a ${selectedIds.length} usuario(s). La recibirán la próxima vez que abran la app.`);
            setTitle('');
            setMessage('');
            setSelectedIds([]);
        } catch (error) {
    ...
    return (
        <div className="space-y-4">
            <div className="bg-graphite-light p-4 rounded-lg space-y-3">
                <h3 className="font-bold text-lg">1. Redactar Notificación</h3>
                <input type="text" placeholder="Título (Ej: ¡No te detengas!)" value={title} onChange={e => setTitle(e.target.value)} required className="w-full bg-graphite border p-2 rounded"/>
                <textarea placeholder="Mensaje (Ej: Estás a un día de extender tu racha. ¡Vamos!)" value={message} onChange={e => setMessage(e.target.value)} rows={3} required className="w-full bg-graphite border p-2 rounded"/>
            </div>

            <div className="bg-graphite-light p-4 rounded-lg">
                <h3 className="font-bold text-lg mb-2">2. Seleccionar Destinatarios</h3>
                <div className="flex gap-2 mb-3">
                    <button onClick={selectAll} className="flex-1 bg-graphite-lighter py-2 rounded-md text-sm font-semibold">Seleccionar Todos</button>
                    <button onClick={() => setSelectedIds([])} className="flex-1 bg-graphite-lighter py-2 rounded-md text-sm font-semibold">Limpiar Selección</button>
                </div>
                <div className="relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={20} />
                    <input type="text" placeholder="Buscar para añadir..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-graphite border rounded py-2 pl-10 pr-4"/>
                </div>
                {searchTerm && (
                    <div className="mt-2 space-y-1 max-h-48 overflow-y-auto">
                        {filteredUsers.map(user => (
                 ...
                 {selectedIds.length > 0 && (
                    <div className="mt-3">
                        <h4 className="text-sm font-semibold text-gray-400">Seleccionados ({selectedIds.length}):</h4>
                         <div className="flex flex-wrap gap-1 mt-2 max-h-24 overflow-y-auto">
                             {users.filter(u => selectedIds.includes(u.id)).map(u => (
                                 <div key={u.id} className="bg-graphite p-1 pr-2 rounded-full flex items-center text-xs">
                                     <UserProfilePicture user={u} className="w-4 h-4 mr-1.5"/>
                                     {u.nombre}
                                     <button onClick={() => toggleSelection(u.id)} className="ml-1.5 text-gray-500 hover:text-white"><X size={12}/></button>
                                 </div>
                             ))}
                         </div>
                    </div>
            ...
            <button onClick={handleSend} disabled={isSending || selectedIds.length === 0 || !title.trim() || !message.trim()} className="w-full bg-accent-lime text-graphite font-bold py-3 rounded-lg flex items-center justify-center disabled:bg-graphite-lighter disabled:text-gray-500">
                {isSending ? 'Enviando...' : <><BellRing size={16} className="mr-2"/> Enviar Notificación a {selectedIds.length} Usuario(s)</>}
            </button>
        </div>
    );
};
...
const Admin: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'users' | 'announcements' | 'messaging' | 'push' | 'customization'>('users');

    const TABS = [
        { id: 'users', label: 'Usuarios', icon: Users },
        { id: 'announcements', label: 'Anuncios', icon: Megaphone },
        { id: 'messaging', label: 'Mensajería', icon: MessageSquare },
        { id: 'push', label: 'Notificaciones Push', icon: BellRing },
    ...
    return (
        <div className="bg-graphite text-white min-h-screen">
            ...
            <main className="p-4">
                ...
                {activeTab === 'push' && <PushNotificationManagement />}
