...
const Explore: React.FC = () => {
    ...
    const filteredRoutines = useMemo(() => {
        ...
        return routines.filter(routine => 
            routine.nombre.toLowerCase().includes(lowercasedSearchTerm) ||
            routine.descripcion.toLowerCase().includes(lowercasedSearchTerm)
        );
    }, [routines, lowercasedSearchTerm]);

    const filteredUsers = useMemo(() => {
        const usersToDisplay = users.filter(user => user.id !== 0); // Exclude support user
        if (!lowercasedSearchTerm) {
            return usersToDisplay;
        }
        return usersToDisplay.filter(user => 
            user.nombre.toLowerCase().includes(lowercasedSearchTerm) ||
            user.username_unico.toLowerCase().includes(lowercasedSearchTerm)
        );
            ...
            {activeTab === 'routines' && (
                <div>
                    <div className="flex items-center text-lg font-bold text-white mb-4">
                        ...
                        <h2>{searchTerm ? 'Resultados de Búsqueda' : 'Rutinas Populares'}</h2>
                    </div>
                    {filteredRoutines.length > 0 ? (
                        ...
                        <div className="text-center py-12 bg-graphite-light rounded-lg">
                            <Search className="mx-auto text-gray-500 mb-2" size={40}/>
                            <h3 className="font-bold text-white">No se encontraron rutinas</h3>
                            <p className="text-gray-400 text-sm mt-1">Intenta con otra palabra clave.</p>
                        </div>
                    ) : null}
                </div>
            )}
            ...
            {activeTab === 'profiles' && (
                <div>
                    <div className="flex items-center text-lg font-bold text-white mb-4">
                        ...
                        <h2>{searchTerm ? 'Resultados de Búsqueda' : 'Perfiles Destacados'}</h2>
                    </div>
                     {filteredUsers.length > 0 ? (
                        ...
                        <div className="text-center py-12 bg-graphite-light rounded-lg">
                            <Search className="mx-auto text-gray-500 mb-2" size={40}/>
                            <h3 className="font-bold text-white">No se encontraron perfiles</h3>
                            <p className="text-gray-400 text-sm mt-1">Intenta con otro nombre o usuario.</p>
                        </div>
                    ) : null}
                </div>
            )}
